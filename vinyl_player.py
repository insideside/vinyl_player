...
//
// Держание его в рабочем состоянии было костылём: оно появилось в 6ce9f4c
// против «плей с локскрина мёртв», а та поломка, как выяснилось позже, шла от
// второго постоянного <audio> из DROPS. Костыль же давал заикание в CarPlay.
//
// После того как previewEl стал ленивым, проверено на устройстве: плей с
// локскрина работает при ac=suspended, и фоновое воспроизведение при спящем
// тракте продолжается (в журнале ms:play -> audio:play -> ac:play>suspend, а
// дальше тики с растущим t).
//
// Переключатель оставлен как запасной выход, трогать его не нужно.
//
// Отдельная ошибка первой попытки усыпления: правило «пока играет, не будим»
// перекрывало вызов внутри обработчика ПАУЗЫ виджета — а он поднимает контекст
// до audio.pause(), то есть ровно в ту секунду, когда сессия иначе
...
var _bgLastFrame = 0;
var _bgPhase = 0;               // своя фаза движения пятен, см. drawBg
...
function drawBg(t) {
  ...
  // Фаза копится сама, а не берётся от абсолютного времени. Отрисовка
  // останавливается, когда окно свёрнуто или фон переведён в статичный, — а
  // время при этом идёт, и пятна при возврате прыгали туда, где они «должны
  // быть». Накопленная фаза продолжает движение ровно с того места, где оно
  // остановилось.
  //
  // Шаг ограничен сотней миллисекунд: после долгой паузы elapsed измеряется
  // минутами, и без ограничения скачок никуда бы не делся. На рабочих частотах
  // (33 и 80 мс между кадрами) ограничение не срабатывает и скорость прежняя.
  _bgPhase += Math.min(elapsed, 100) * 0.0002;
  var s = _bgPhase;
...
var _frzT = -1, _frzAt = 0, _frzLogged = false, _frzGen = -1;

function freezeCheck() {
  if (!audio || audio.paused || audio.ended) { _frzT = -1; _frzLogged = false; return; }
  // Смена трека обнуляет часы, и без этой проверки сторож принимал начало
  // нового трека за замирание: сравнивал t=0.7 нового с t=82 прежнего. То же
  // самое отдельно учитывает resumeWatch — там сверка идёт по _trackSrcGen.
  if (_frzGen !== _trackSrcGen) { _frzGen = _trackSrcGen; _frzT = -1; _frzLogged = false; }
